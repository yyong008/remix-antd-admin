    ...
    <>
      ...
      <ProFormDependency key="typeMode" name={["type"]} ignoreFormListField>
        {({ type }) => {
          if (type === 1) {
            return (
              <TypeDir menuNotPerm={innerMenuNotPerm} form={form} icon={record?.icon}></TypeDir>
            );
          }
          if (type === 2) {
            return (
              <TypeMenu menuNotPerm={innerMenuNotPerm} form={form} icon={record?.icon}></TypeMenu>
            );
          }
          if (type === 3) {
            return <TypePermission menuNotPerm={innerMenuNotPerm}></TypePermission>;
          }
        }}
      </ProFormDependency>
    </>
