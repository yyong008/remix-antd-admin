...
function DeleteAction({ record, refetch }: any) {
  ...
  return (
    <Form>
      <Popconfirm
        title={m.system_menu_confirm_delete()}
        onConfirm={async () => {
          const ids = [record.id];
          await deleteMenus.mutateAsync({ ids });
          refetch?.();
          message.success(m.system_delete_success());
        }}
      >
        <Button type="link" danger icon={<DeleteOutlined />} loading={deleteMenus.isPending} />
      </Popconfirm>
    </Form>
  );
}

export type Status = { color: string; text: string };
  ...
  if (!record) {
    return (
      <Empty
        ...
        description={m.system_menu_detail_empty()}
        style={{ paddingTop: 24 }}
      />
    );
  }
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      >
        ...
        <Space wrap style={{ flexShrink: 0 }}>
          ...
          <DeleteAction record={r} refetch={refetch} />
        </Space>
      </div>
      ...
      >
        <Descriptions.Item label={m.system_menu_detail_type()}>
          <MenuType type={(r.type ?? 1) as 1 | 2 | 3} />
        </Descriptions.Item>
        <Descriptions.Item label={m.system_status()}>
          <StatusType status={(r.status as any).color === "green" ? 1 : 0} />
        </Descriptions.Item>
        <Descriptions.Item label={m.system_menu_detail_tags()}>
          <Space wrap size={[4, 4]}>
        ...
        <Descriptions.Item label={m.system_menu_detail_order()}>
          {r.orderNo ?? "—"}
        </Descriptions.Item>
        <Descriptions.Item label={m.system_menu_detail_path()}>
          {r.isLink ? (
        ...
        <Descriptions.Item label={m.system_menu_detail_path_file()}>
          <Typography.Text style={{ wordBreak: "break-all", fontSize: 14 }}>
            {r.path_file || "—"}
          </Typography.Text>
        </Descriptions.Item>
        <Descriptions.Item label={m.system_menu_detail_permission()}>
        ...
        <Card
          ...
          title={
            <span style={{ color: "var(--ant-color-text-heading)", fontWeight: 600 }}>
              {m.system_menu_tree_title()}
              <Typography.Text
                type="secondary"
                style={{ marginLeft: 8, verticalAlign: "middle", fontSize: 12, fontWeight: 400 }}
              >
                {m.system_menu_tree_items({ total: String(total ?? 0) })}
              </Typography.Text>
            </span>
          }
          extra={
