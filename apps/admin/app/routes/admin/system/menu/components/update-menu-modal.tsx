...
export function UpdateMenuModal({ trigger, record, refetch, menuNotPerm }: MenuModalProps) {
  ...
  return (
    <ModalForm
      {...menuModalFormLayout}
      preserve={false}
      title={record?.id ? m.system_menu_modal_update() : m.system_menu_modal_create()}
      ...
      onOpenChange={(c) => {
        ...
        form.setFieldsValue({ ...record, parent_menu_id, type: Number(record.type) });
      }}
      trigger={
        trigger ??
        ((
