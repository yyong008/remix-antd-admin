...
import { departments, menuRoles, menus, roles, userRoles, user } from "./schema";
...
async function ensureDepartment() {
  const existing = await db.select().from(departments).where(eq(departments.name, "Root")).limit(1);
...
async function ensureMenuItem(data: MenuSeed, parentMenuId: number | null) {
  const conditions = [eq(menus.name, data.name), eq(menus.type, data.type)];
...
async function ensureUser({
  ...
  const existing = await db.select().from(user).where(eq(user.email, email.toLowerCase())).limit(1);
  const found = existing[0];
  if (found) return found;
  ...
  const created = await db.select().from(user).where(eq(user.email, email.toLowerCase())).limit(1);
  if (!created[0]) {
...
async function ensureUserRole(userId: string, roleId: number) {
  ...
  const found = existing[0];
  if (found) return found;
  const created = await db.insert(userRoles).values({ userId, roleId }).returning();
  return created[0];
}
...
async function ensureMenuRole(menuId: number, roleId: number) {
  ...
  const found = existing[0];
  if (found) return found;
  const created = await db.insert(menuRoles).values({ menuId, roleId }).returning();
  return created[0];
}

async function ensureRoleMenus(roleId: number, menuIds: number[]) {
  if (!menuIds.length) return;
  const existing = await db.select().from(menuRoles).where(eq(menuRoles.roleId, roleId));
...
function collectDescendantIds(
  allMenus: { id: number; parentMenuId: number | null }[],
  rootIds: number[],
) {
