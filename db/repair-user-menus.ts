...
function readOptions(): RunOptions {
  ...
  const assignAllMenus = String(process.env.ASSIGN_ALL_MENUS ?? "true").toLowerCase() === "true";
...
async function main() {
  ...
  const roleRows = await db.select().from(roles).where(eq(roles.value, roleValue)).limit(1);
  ...
  const userRoleRows = await db.select().from(userRoles).where(eq(userRoles.userId, authUser.id));
  const roleIds = userRoleRows.map((row) => row.roleId);
  const menuRoleRows = roleIds.length
    ? await db.select({ count: count() }).from(menuRoles).where(inArray(menuRoles.roleId, roleIds))
    : [];
