  ...
  return (
    <Form name="register" onFinish={onFinish} size="large" layout="vertical">
      ...
      <Form.Item name="email" rules={[{ type: "email", message: "Please enter a valid email." }]}>
        <Input prefix={<MailOutlined />} placeholder="Email" autoComplete="email" allowClear />
      </Form.Item>
      <Form.Item
        name="password"
        rules={[
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3">
        <LogoImg />
        <div>
          <Typography.Title level={4} style={{ margin: 0, color: "var(--mkt-text)" }}>
            Remix Antd Admin
          </Typography.Title>
      ...
      <div className="flex items-center justify-between text-sm">
        <Typography.Text style={{ color: "var(--mkt-muted)" }}>
          ...
          <Link to={href("/:locale?/auth/login", { locale })} className="text-[var(--mkt-accent)]">
            Sign in
          </Link>
        </Typography.Text>
