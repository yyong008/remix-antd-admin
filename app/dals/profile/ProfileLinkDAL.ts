...
export function createProfileLinkDAL(db: DrizzleD1Database) {
  async function getCount(userId: string) {
    const rows = await db.select({ count: count() }).from(links).where(eq(links.userId, userId));
  ...
  async function update({ id, ...data }: any) {
    const updated = await db.update(links).set(data).where(eq(links.id, id)).returning();
    return updated[0];
  }
