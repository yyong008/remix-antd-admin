...
/** Columns always present on `profile_link` (migration `0000`). */
const linkRowBase = {
  id: links.id,
  name: links.name,
  url: links.url,
...
/** DB missing `0001_profile_link_timestamps` — no `created_at` / `updated_at` columns. */
function isMissingProfileLinkTimestampColumnsError(err: unknown): boolean {
  ...
  // Some drivers only surface the SQL text:
